<template>
<div v-if="showIt" class="safari-how2 fixed-bottom">
  <div class="modal-header">
    <h5 class="modal-title" id="modal-basic-title">Zum Homescreen hinzufügen</h5>
    <button
      type="button"
      class="close"
      data-dismiss="modal"
      aria-label="Close"
      @click="close()">
      <span aria-hidden="true">&times;</span>
    </button>
  </div>
  <div class="modal-body">
    <ol>
      <li>
        Verwende den Share-Button
        <img class="nudgeImage" height="22" alt="Share Button Image"
        src="data:image/svg+xml;base64,
        dmc+"> {{a2hs.whereIsShare}} im Browser.

      </li>
      <li>
        Scrolle nach links (falls notwendig), um den Button "Zum Home-Bildschirm"
        <img class="nudgeImage" height="22" alt="Add to Home Screen button Image"
        src="data:image/svg+xml;base64,"> zu finden.
      </li>
    </ol>
  </div>
</div>
</template>

<script>
export default {
  name: 'A2HSSafari',
  props: {
    showIt: {
      type: Boolean,
      required: true
    }
  },
  methods: {
    close () {
      this.showIt = false
    }
  }

}
</script>
<style lang="sass">
.safari-how2
  white-space: normal
  border: 1px solid rgba(0, 0, 0, .5)
  padding: 10px
  margin: 10px 5px 80px 5px
  border-radius: .25rem
  background-color: #ffc107
  z-index: 9999

.nudgeImage
  position: relative
  top: 0px
</style>
